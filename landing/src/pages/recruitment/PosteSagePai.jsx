...
export default function PosteSagePai(){
    ...
    const actuallisationINfoSagepai = async ()=>{
        const data = await getData(url_recrutement +nameE+"/reinitialisation")
        if (data == true) {
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 border-b border-gray-200">
                            <tr>
                                ...
                                <th class="tr-thead">Emplois Type</th>
                                <th class="tr-thead">Service</th>
                                <th class="tr-thead">Unite</th>
                                <th class="tr-thead">Catgorie</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200">
                            {data.map((value,index)=>(
                                ...
                                <tr index={index} className={value.active==4 ?"bg-gray-50  hover:bg-gray-100":" hover:bg-gray-50"}>
                                    <td class="px-6 py-4"><span class={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium ${textbackground[index]}`}>{value.code}</span></td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{value.intitule}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{value.nameEmploiType}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{value.nameService}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{value.nameUnite}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{value.nameCategorie}</td>
                                </tr>
                                </>
                            ))}
                        </tbody>
